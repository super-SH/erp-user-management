...
import { separateByFeature } from '@/app/utils';

type RoleFormProps = { featureActions: ActionWithFeature[] } & (
  | {
...
function RoleForm({
  ...
  const featureActionsSeparated = separateByFeature(featureActions);
        ...
        <FormField
          ...
          render={() => (
            <FormItem className='flex flex-col'>
                  ...
                  render={({ field }) => (
                    ...
                    >
                      <FormControl>
                        <Checkbox
                          className='size-6'
                          checked={featureActions.every((permission) =>
                            field.value.includes(permission.id)
                          )}
                          onCheckedChange={(checked) => {
                            const ids = featureActions.map((item) => item.id);
                            field.onChange(
                              checked
                                ? [...ids]
              ...
              {Object.entries(featureActionsSeparated).map(
                ([feature, actionArrays]) => (
